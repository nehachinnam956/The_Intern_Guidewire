...
export default function ClaimsPage({ rider, activePolicy, claimHistory, showToast, onClaimFiled }) {
  ...
  }, [activePolicy]); // eslint-disable-line
    ...
    try {
      const response = await fetch(`${API_BASE}/claims/file`, {
        ...
        body: JSON.stringify({
          ...
          store_id: activePolicy.store_id,
          simulate: true
        }),
      });
      ...
      while (true) {
        ...
        for (const line of lines) {
          try {
            ...
            // ── Safety check — only add if data has msg ──
            if (data && data.msg) {
            ...
            if (data && data.error) {
              showToast('Claim error: ' + data.error, 'error');
              setFiring(false);
            }
          } catch(e) {
      ...
      console.log('Backend claim failed, simulating locally:', e.message);
  ...
  function simulateLocally(trigger) {
    ...
    const interval = setInterval(() => {
      if (i < mockLogs.length) {
        const logEntry = mockLogs[i];
        if (logEntry && logEntry.msg) {
          setLogs(prev => [...prev, logEntry]);
          setPipelineStep(logEntry.step || 0);
        }
        i++;
      } else {
        ...
        const claim = {
          ...
          store_name: activePolicy?.store_name
        };
          ...
          {!rider ? (
            ...
            <>
                ...
                <div style={{background:'rgba(255,71,89,0.08)',border:'1px solid rgba(255,71,89,0.3)',borderRadius:10,padding:14,marginBottom:20,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <div>
                    <div style={{fontSize:13,fontWeight:700,color:'var(--red)',marginBottom:4}}>
                      ⚡ Live Disruption Detected at Your Store
                    </div>
                    <div style={{fontSize:11,color:'var(--muted2)'}}>
                      {liveWeather.active_triggers.map(t => t.label).join(', ')}
                    </div>
                  </div>
                  <button className="btn btn-danger btn-sm"
                    onClick={() => fileClaim(TRIGGERS.find(t => t.id===liveWeather.active_triggers[0].type) || TRIGGERS[0])}>
                    Auto-Claim Now
                  </button>
                </div>
              )}
              ...
              <div className="trigger-grid">
                {TRIGGERS.map(t => (
                  <button key={t.id}
                    className={`trigger-card ${currentTrigger?.id===t.id && firing ? 'active-trigger' : ''}`}
                    onClick={() => !firing && fileClaim(t)}
                    disabled={firing}
              ...
              {firing && currentTrigger && (
                <div className="pipeline">
                  ...
                  <div className="log-box" ref={logRef}>
                    {logs.length === 0 && (
                      <span style={{color:'var(--muted)'}}>Initialising pipeline...</span>
                    )}
                    {logs.filter(l => l && l.msg).map((l, i) => (
                  ...
                  {payoutDone && (
                    <div className="payout-banner">
                      <div className="payout-banner-left">
                        ...
                        <div className="payout-banner-sub">
                          Zero-touch · {payoutDone.txId} · No forms required
                        </div>
                      </div>
        ...
        <div>
          {!claimHistory || claimHistory.length === 0 ? (
            <div style={{textAlign:'center',padding:'40px',color:'var(--muted2)',fontSize:13}}>
              No claims yet. Trigger a disruption event to file your first claim.
            </div>
          ) : (
            <div className="card">
              <div className="table-wrap">
                <table>
                  ...
                  <tbody>
                    {claimHistory.filter(c => c).map(c => (
                      <tr key={c.id}>
                        <td style={{textTransform:'capitalize'}}>
                          {TRIGGERS.find(t => t.id===c.trigger_type)?.icon} {c.trigger_type}
                        </td>
                        <td style={{color:'var(--muted2)'}}>{c.store_name}</td>
                        <td style={{color:'var(--green)',fontWeight:700}}>₹{c.payout_amount}</td>
                        <td>
                          <span className={`badge ${c.status==='approved'?'b-g':c.status==='rejected'?'b-r':'b-y'}`}>
                            {c.status}
                          </span>
                        </td>
        ...
        <div style={{maxWidth:520}}>
          <div className="card">
            <div className="card-hd">
              <span className="card-title">⚠️ Coverage Exclusions</span>
            </div>
